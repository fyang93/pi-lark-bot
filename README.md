...
**安装、加载、连接和重启都不会自动监听。** 每次需要手动 `on`。

`connect` 支持扫描终端中显示的官方二维码创建新机器人，**或直接扫码后在官方页面选择已有机器人连接**；后者由应用所有者扫码确认，并会增量申请本扩展所需的权限和事件。也可输入已有 App ID / App Secret。二维码失效前可用 Esc 取消；终端过窄导致二维码换行时请加宽，页面 URL 也会同时显示。密钥隐藏输入，只保存到当前项目，不进入聊天上下文、启动参数或系统钥匙串。
...
3. 开通以下权限并完成必要的审批/发布：
   ...
   - `im:message:update`
4. 如需 `/model` 的按钮菜单，在应用的回调订阅中启用 `card.action.trigger`；重新扫码连接已有应用时会增量申请该回调。
5. 群聊使用前，把机器人添加到群中。
...
| `/lark-bot off` | 停止监听，关闭本扩展的 pane，保留历史 |

聊天中也支持：

| 命令 | 行为 |
| --- | --- |
| `/new` | 清除当前私聊用户的会话；在群聊中仅清除当前群的共享会话。 |
| `/model` | 显示可用模型和当前会话模型。 |
| `/model provider/model` | 切换当前私聊或群聊会话的模型；下一条消息会在保留的历史中使用新模型。 |
...
- 工具名称、执行状态和公开回答草稿节流更新；完成时直接将同一条流式消息替换为最终回复，不产生第二个消息气泡。不转发 thinking、工具参数或原始工具输出。
