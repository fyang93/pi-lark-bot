...
事件的接收、重连、@机器人的识别和消息归一化全部交给飞书 SDK 的 channel（`createLarkChannel`），本扩展不自己维护连接状态、不解析 mention、不判断消息类型。群聊"必须 @机器人"、私聊"直接放行"这两条规则由 `policy: { dmMode: "open", requireMention: true }` 表达；SDK 自带的连续消息合并被显式关闭，因为每条消息各有自己的会话和回复目标。

**发给机器人的消息一定会有回应。** 私聊的全部消息、群里真正 @机器人的消息，即使内容无法执行也会收到明确答复：

| 情况 | 回应 |
| --- | --- |
| 非文字消息（图片、文件等） | 告知暂时只能处理文字 |
| 内容为空 | 告知把要做的事写清楚 |
| 本地状态写入失败 | 告知该消息未执行、请重新发送 |

SDK 因策略拒绝的消息、以及连接错误，都会在本机 pi 中提示，不会无声消失。
