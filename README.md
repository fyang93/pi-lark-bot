...
> [!WARNING]
> 这是没有用户或群白名单的基础版。任何能私聊机器人、或在群中 @机器人的成员，都能触发运行 pi，并因而使用运行 pi 机器上的工具权限。请使用测试应用、限制开放平台中的应用可用范围，并不要把它当作权限沙箱；所有会话共享同一个项目文件。
...
- **私聊**按用户 `open_id` 隔离；**群聊**按 `chat_id` 隔离，同一群成员共享群上下文。
- 首条消息创建 pane，后续消息复用；同一会话按 FIFO 执行，不同会话可并行。
- 本地可直接查看 pane、输入消息及处理权限确认；本地输入的回答不会自动转发回飞书/Lark。
- 远程消息通过私有 Unix socket 交给 pi，不会被键入 shell 或终端。群消息以 `user_id: message` 形式提交，区分发言者。
...
扩展只处理人类用户发来的文本：私聊文本会直接处理；群聊消息必须真正 @当前机器人。@其他用户、@所有人、图片和文件均不会触发。启动时扩展会读取机器人自身的 `open_id`，用于校验 mention 身份。
...
认证目录和文件权限分别为 `0700`、`0600`，并会自动写入项目 `.pi/.gitignore`。认证和会话均为本地明文：不要分享或提交它们。模型认证仍使用 pi 自己的配置，且没有全局 bot 认证回退；旧配置中的白名单字段不再生效。
